...
(require (only-in racket add1 foldr range)
...
(define newref
  (let ((next-ref 0))
    (lambda (val)
      (set! next-ref (add1 next-ref))
      (when (> next-ref (vector-length the-store))
        (set! the-store
              (vector-append the-store (make-vector 10 #f))))
